    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if (array_key_exists("id", $_GET)) {
                ...
                if (array_key_exists("idcotizacion", $_GET)) {
                    $cotizacion = Cotizacion::find()->where(['idcotizacion' => $_GET["idcotizacion"]])->one();
                    $proyecto = Proyectos::find()->where(['idProyecto' => $cotizacion->idProyecto])->one();

                    $cliente = Cliente::find()->where([
                        'rutCliente' => $proyecto->rutCliente
                    ])->one();

                    $model->idProyecto = $cotizacion->idProyecto;
                    $model->evento = $proyecto->nombreProyecto;
                }
            }
        }
