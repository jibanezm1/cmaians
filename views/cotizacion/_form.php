                                ...
                                <div style="text-align: center;" class="col-sm-5">
                                    <?= $form->field($modelAddress, "[{$i}]idproducto")->widget(Select2::className(), [
                                        'data' => ArrayHelper::map(app\models\Producto::find()->orderBy(['nombreProducto' => SORT_ASC])->all(), 'idproducto', function ($model) {
                                            return $model->nombreProducto . "(P: $" . $model->precioTienda . ")";
                                        }),
                                    ])->label(false); ?>
                                </div>
                                <div style="text-align: center;" class="col-sm-2">
        ...
        $.post('../cliente/buscador', {
            ...
            function(returnedData) {
                ...
                if (returnedData) {
                    ...
                    $('#cliente-regioncliente').val(obj.regionCliente);
                    $('#cliente-regioncliente').select2().trigger('change');

                    $('#cliente-comunacliente').val(obj.comunaCliente);
                    $('#cliente-comunacliente').select2().trigger('change');
